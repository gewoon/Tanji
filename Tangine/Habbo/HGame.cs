    ...
    public class HGame : ShockwaveFlash
    {
        ...
        public void BypassRemoteHostCheck()
        {
            ...
            foreach (ASTrait trait in habboCommMngr.Traits)
            {
                ...
                hostValueSlotObjName = trait.QualifiedName.Name;
                hostValueSlotObjTypeIndex = trait.QualifiedNameIndex;
                break;
            }
            ...
            int getPropertyObjTypeIndex = abc.Constants
                .GetQualifiedName("getProperty").Index;
        ...
        /// <summary>
        /// Returns the hardcoded RSA key pair in a <see cref="Tuple{T1, T2}"/>(Modulus, Exponent).
        /// </summary>
        public Tuple<string, string> RetrieveRSAKeys()
        {
            ABCFile abc = ABCFiles[2];
            ...
            int rsaKeyTypeIndex = abc.Constants.GetQualifiedName("RSAKey").Index;
            ...
            using (var inCode = new FlashReader(method.Body.Bytecode))
            {
                while (inCode.Position != inCode.Length)
                {
                    ...
                    if ((int)values[0] != rsaKeyTypeIndex) continue;

                    // Read Modulus
                    values = inCode.ReadValuesUntil(OPCode.PushString);
                    modulus = abc.Constants.Strings[(int)values[0]];

                    // Read Exponent
                    values = inCode.ReadValuesUntil(OPCode.PushString);
                    exponent = abc.Constants.Strings[(int)values[0]];
        ...
        public void ReplaceRSAKeys(int exponent, string modulus)
        {
            ...
            using (var inCode = new FlashReader(method.Body.Bytecode))
            {
                ...
                int keyObfuscatorTypeIndex = abc.Constants.GetQualifiedName("KeyObfuscator").Index;
        ...
        public string GetMessageHash(ASClass messageClass)
        {
            ...
            using (var hashOut = new FlashHasher())
            {
                ...
                if (!messageClass.Instance.QualifiedName.Name.EndsWith("Composer") &&
                    ...
                    foreach (ASReference msgReference in msgReferences)
                    {
                        if (!isOutgoing)
                        {
                            string fromClassName =
                                msgReference.FromClass.Instance.QualifiedName.Name;
        ...
        public ASClass GetIncomingMessageParser(ASClass messageClass)
        {
            ...
            try
            {
                ASInstance incomingMsgSuperInstance = abc.FindFirstInstanceByName(
                    incomingMsgInstance.SuperName.Name);
        ...
        protected void Write(FlashHasher hashOut, ASClass messageClass, bool isOutgoing)
        {
            if (isOutgoing)
            {
                string messageName = messageClass.Instance.SuperName.Name;
                string messageSuperName = messageClass.Instance.SuperName.Name;
            ...
            else
            {
                ...
                string parserObjName = incomingMsgParser.Instance.QualifiedName.Name;
        ...
        protected void FindHabboMessageClasses(ASClass habboMessageClass)
        {
            ...
            int incomingMapTypeIndex = habboMessageClass.Traits[0].QualifiedNameIndex;
            int outgoingMapTypeIndex = habboMessageClass.Traits[1].QualifiedNameIndex;
                                ...
                                if (msgReference.FromMethod == null && multinameStack.Count > 0)
                                {
                                    ...
                                    foreach (ASTrait trait in container.Traits)
                                    {
                                        switch (trait.TraitType)
                                        {
                                            ...
                                            {
                                                if (trait.QualifiedName.Name != bottomName.Name) continue;
